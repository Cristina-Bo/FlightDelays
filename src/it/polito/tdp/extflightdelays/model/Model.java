...
public class Model {
	...
	public void creaGrafo(int distanzaMedia) {
		...
		// aggiungo i vertici(ho la mappa popolata)
		Graphs.addAllVertices(grafo, aIdMap.values());
		
		
		//aggiungo archi
		for(Rotta rotta : dao.getRotte(aIdMap, distanzaMedia)) {
			// controllo se esiste gia' un arco tra i due areoporti
			...
			else {//l'arco esisteva gia'
				
				double peso = grafo.getEdgeWeight(edge);
				double newPeso = (peso + rotta.getDistanzaMedia())/2;
				System.out.println("Aggiorna peso!Peso vecchio:"+peso+" peso nuovo:"+newPeso);
